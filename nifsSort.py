...
import sys
# we import some functions that we made ourselves.
# def getUrlFiles(url,tag):
#   """ Use this to access xml at a given web address. Maybe it should only have a url parameter.
#
#   It is used to parse for <file> tags and the <filename> tag between those <file> tags.
#
#   It returns a list of file names in string form from the supplied URL.
#   """
#
#
# def getFitsHeader(fitsFile, fitsKeyWords):
#   """
# >>>>>>>>>>MArie :Can we just delete the part commented above ?
...
def start(program, date, dir, tel, over, copy, sort):
    ...
    Parameters:
        ...
        dir: raw files directory specified with -q at command line
        tel: boolean. Default: True. specified with -t at command line. If yes no telluric corrections will be executed.
        over: boolean. Default: False. Specified with -o at command line. If yes old files will be overwritten during data reduction.
        copy: Boolean. Default: True. Specified with -c at command line. If yes data will not be copied from gemini network. Note this only works in gemini network.
              The parameter should not be named -c AND --nocopy. It should be named --copy with a default set to false.
              Right now it is named nocopy with a default set to True. You must set this parameter to False to look for data on the Gemini network.
              Perhaps it should be -n --nocopy with default of True or -c --copy with default of False.
        sort: boolean. Default false. Specified with -s at command line. If True data will not be sorted.
    """
    # Question- does this use the same logfile as main.py or does it use something else entirely?
# >>>>>>>>>>MArie : it uses the main.log but my feeling is that the only thing it writes is the paths to the science and telluric data. 
# >>>>>>>>>>MArie : e. g. "2015/01/22 17:00:10   I am sorting the data" ; 	"obsDirList : ['/home/mbussero/pipe1/SW1609+5423/20140908/J/obs77']"; " 2015/01/22 17:00:10  telDirList : ['/home/mbussero/pipe1/SW1609+5423/20140908/J/Tellurics/obs79']"
    ...
    # If not nocopy and not nosort
    # aka if copy and sort
    # Data will be copied from /net/wikiwiki/dataflow from GEMINI network and sorted.

    # By default, this block of code will not run.
# >>>>>>>>>>MArie : This part will be executed ONLY iF -c and -s  are NOT specified in the comand line
...
# >>>>>>>>>>MArie : This part will be executed ONLY iF -c  is  specified in the comand line
...
# >>>>>>>>>>MArie : This part will be executed ONLY iF -s  is  specified in the comand line
        ...
        # when a date is given (looks for data using http://fits/xmlfilelist/summary/NIFS)
            ...
            allfilelist, arclist, arcdarklist, flatlist, flatdarklist, ronchilist, objDateList, skylist, telskylist, obsidDateList = makeSortFiles(dir)
            obsDirList, calDirList, telDirList = getPaths(allfilelist, objDateList, dir)
        # If a raw files directory was specified:
        if dir:
            allfilelist, arclist, arcdarklist, flatlist, flatdarklist, ronchilist, objDateList, skylist, telskylist, obsidDateList = makeSortFiles(dir)
            obsDirList, calDirList, telDirList = getPaths(allfilelist, objDateList, dir)
    ...
    # This block is followed by default if only the -q option of a raw file directory is specified when running Main.py
    elif dir and sort:
        print 'block 163 followed'
        allfilelist, arclist, arcdarklist, flatlist, flatdarklist, ronchilist, objDateList, skylist, telskylist, obsidDateList = makeSortFiles(dir)
        objDirList, obsDirList, telDirList = sortObs(allfilelist, skylist, telskylist, dir)
        calDirList = sortCals(arcdarklist, arclist, flatlist, flatdarklist, ronchilist, objDateList, objDirList, obsidDateList, dir)
        # if a telluric correction will be performed sort the science and telluric images based on time between observations
...
def getProgram(program, date, over):
    ...
    # At the URL (without program date and object) is a lot of XML with details for quite a few files.
...
# >>>>>>>>>>MArie : the path as changed since
...
def makeSortFiles(dir):
	...
	Parameters: dir: Directory containing raw .fits files. Specified at command line with -q.

    Returns: allfilelist: list of all .fits image files. ['filename.fits', 'filename2.fits']

			 arclist: list of .fits image files. ['filename.fits', 'filename2.fits']

			 arcdarklist: list of .fits image files. ['filename.fits', 'filename2.fits']

			 flatlist: list of .fits image files. ['filename.fits', 'filename2.fits']

			 flatdarklist: list of .fits image files. ['filename.fits', 'filename2.fits']

			 ronchilist: list of .fits image files. ['filename.fits', 'filename2.fits']

			 skylist: list of .fits filenames. ['filename.fits', 'filename2.fits']

			 telskylist: list of .fits filenames. ['filename.fits', 'filename2.fits']

			 ######### Different Format ############

			 objDateList: list of obj, date pairs. [['obj', 'YYYYMMDD'], ['obj2','YYYYMMDD']]

			 obsidDateList: list of date, obsid pairs. [['YYYYMMDD','obsid'],['YYYYMMDD', 'obsid']]
	...
	sciDateList = [] # Format: list of dates. ['YYYYMMDD', 'YYYYMMDD']

    # Save current working directory into path
    path = os.getcwd()

    # Store the path to the raw files in Raw.

    # If user gave a non-empty string for the raw directory path use that:
    if dir:
        Raw = dir

	# Otherwise use a default (possibly for tests?) that raw files are in cwd+'/Raw'
	else:
        Raw = path+'/Raw'


    # Change directory to where the raw files are located.
    os.chdir(Raw)

    # make a list of all raw files
    rawfiles = glob.glob('N*.fits')

    # sort the files into lists (allfilelist and cal lists)

    # We iterate through and open the raw .fits files. We read the header information of the
    # .fits file into local variables. We then add filenames to appropriate lists based on thoses

	# Everytime you see entry think of an N*.fits filename.
    for entry in rawfiles:

        # Here we use pyfits to open .fits files. We process one file at a time.
        #
        # Note: pyfits.open returns a list-like object.
        # This should also be changed to something besides header.
        header = pyfits.open(entry)   # header is now an object reference. Note that this is to the ENTIRE .fits file. Now we need to grab just the juicy bits at the top.

        # We're transfering the header data from the .fits file to individual variables.

        # return a copy of the string with whitespacecharacters removed.
        #
        #
        # We need header[0] because header[0] is text data and header[1] is image data.
		# .strip() strips leading or trailing whitespacecharacters
		obstype = header[0].header['OBSTYPE'].strip()    # determines which files are ARCS
        # Stores the observation id in ID
        ID = header[0].header['OBSID']           # 1st sorting criterion
        # stores the date, removing '-' (hyphens) first.
        ...
        if obstype == 'OBJECT' and (obsclass == 'science' or obsclass == 'acq' or obsclass == 'acqCal' or obsclass == 'partnerCal'):
            ...
            # if the offsets are outside a circle of 5.0 units in radius- LOOK THIS OVER
            if obsclass == 'science':
                if rad > 3.0:
                    skylist.append(entry)
    ...
    for i in range(len(rawfiles)): # Iterate through the list of raw .fits files
        ...
        if obsclass == 'science':
            list1 = [obj, date]
            if not objDateList or not objDateList[-1]==list1: # If objDateList is empty OR it is an NOT an identical entry to the last entry in the list
                objDateList.append(list1)
    n=0

    # This part runs through the 'flat field images' and if it is not the first image or not a duplicate of the previous image,
    # We append the image date/obsid IF the date is already in the science date list
    # Else we append the first, second, third, etc... date in sci date list with the object id
    for flat in flatlist:  # I don't totally understand this part yet.
...
def sortObs(allfilelist, skylist, telskylist, dir):
    ...
    """ sorts the science images, tellurics and acquisitions from the raw files directory into created directories
	based on date, grating, obsid, and obsclass. This is done by default when not using the Gemini network.

	Parameters:
		allfilelist: list of all raw image filenames
		skylist: list of raw sky images
		telskylist: list of raw teluric images
		dir: path to directory where raw images are located

	Returns:
		objDirList: paths to objects (sorted by date). List of format path/objectname/date.
		obsDirList: paths to observations. List of format path/objectname/date/grating/obsid.
		telDirList: paths to tellurics. List of format path/objectname/date/grating/Tellurics/obsid.

	Creates a directory structure and copies raw image files to appropriate location.
			...
			--->objname/
				--->date/
					--->grat/
						--->obsid/
							---> *.fits image files
							--->objlist: text file containing names of images
							--->tellist: text file containing names of images
						--->Tellurics/
							--->obsid/
								---> *.fits image files
								--->tellist: text file containing names of images
								--->skylist: text file containing names of images
						--->Acquisitions/
							---> *.fits image files
    ...
    for entry in allfilelist:
        ...
        if obsclass=='science':
			# create the path/objectname/date directories.
            # create the object directory (name of target) in the current directory
			# and append unique object directories to objDirList
    ...
    for entry in allfilelist:
        ...
        grat = header[0].header['GRATING'][0:1]  # Trim the string with indexing
        date = header[0].header[ 'DATE'].replace('-','')         # 1st CAL sorting criterion
        obsclass = header[0].header['OBSCLASS']  # used to sort out the acqs and acqCals in the trap
        obj = header[0].header['OBJECT'].replace(' ','')
    ...
    for i in range(len(allfilelist)):
		...
		# Store info from next file in list to check later if this is acquisition/science pair
        if i!=len(allfilelist)-1:
        ...
        if obsclass=='science':
			# If science copy to appropriate directory. Sort in order by object, date, grating, and observation id.
			# Fix next line
			objDir = path+'/'+obj
            shutil.copy(Raw+'/'+allfilelist[i], objDir+'/'+date+'/'+grat+'/obs'+obsid+'/')
            # create an objlist in the relevant directory. objlist is a text file list of objects.
            if allfilelist[i] not in skylist:
                writeList(allfilelist[i], 'objlist', objDir+'/'+date+'/'+grat+'/obs'+obsid+'/')
            # create a skylist in the relevant directory. This makes a text file at in each /objDir/date/grat/obsid/ directory

            if allfilelist[i] in skylist:
                writeList(allfilelist[i], 'skylist', objDir+'/'+date+'/'+grat+'/obs'+obsid+'/')

		# Handle Tellurics data
        if obsclass=='partnerCal':
            for objDir in objDirList:
				# check to see if telluric data has a date associated with it.
				# Splits path to object into list. Eg. path/obj/date/grat/obs/obsid/ becomes ['path', 'obj', 'date', 'grat', 'obs', 'obsid', '']
                ...
                if date in tempDir:
					...
					# Copy the appropriate raw data to the Tellurics/obsid/ directory
        ...
        if obsclass=='acq' and obsclass2=='science':
            ...
            if not os.path.exists(path+'/'+obj2+'/'+date+'/'+grat+'/Acquisitions/'):
				...
				# obj2 is name of science object.
...
def sortCals(arcdarklist, arclist, flatlist, flatdarklist, ronchilist, objDateList, objDirList, obsidDateList, dir):
    ...
    """ sort calibrations into the appropriate directory based on date
		Parameters:
			...
			objDateList: list of object, date pairs. [['obj', 'YYYYMMDD'], ['obj2, YYYYMMDD']]
			objDirList: list of paths to objects. Format: ['path/objname/date']
			obsidDateList: list of date, obsid pairs. [['YYYYMMDD', 'obsid'],['YYYYMMDD','obsid']]
			dir: directory of raw image files. Specified with -q at command line in Main.py.

		Returns:
			calDirList: list of paths to Calibration directories. ['cwd/obj/date/Calibrations']
	"""
    calDirList = []
    ...
    for item in objDateList:
		# mkdir /path/objname/date/Calibrations
		if not os.path.exists(path1+'/'+item[0]+'/'+item[1]+'/Calibrations'):
            ...
            calDirList.append(path1+'/'+item[0]+'/'+item[1]+'/Calibrations')
            for list in filelist: # Why is this here?
...
def getPaths(allfilelist, objDateList, dir):
    ...
    Parameters:
        allfilelist: a list of all the files we hope to process.
        objDateList:
    ...
    # We save the current working directory in path so we can come back to it later
    path = os.getcwd()
    # We see if a directory for the raw files was specified. If not, we assume they exist in our cwd + '/Raw'.
    if dir:
        Raw = dir
    else:
        Raw = path+'/Raw'
    ...
    for entry in allfilelist:
        # Open each file found at path/filename or path/Raw/filename
        header = pyfits.open(Raw+'/'+entry)

        # Read the header information of the .fits file into local variables.
        obstype = header[0].header['OBSTYPE'].strip()
        obsid = header[0].header['OBSID']
        ...
        if obsclass=='science':
            objDir = path+'/'+obj
            # Path one is path/obj/date/grat/obs[last_three_characters_of_obs_id_hyphens_removed]
            path1 = (objDir+'/'+date+'/'+grat+'/obs'+obsid[-3:].replace('-',''))
            # If obsDirList is empty or it is not a duplicate of the last entry(most recent) of obsDirList, append our special unique path to obsDirList.
    ...
    for entry in allfilelist:
        ...
        if obsclass == 'partnerCal':
            for obsDir in obsDirList:
                # This splits the path name into a list of its components. Eg /this/is/a/path becomes ['this', 'is', 'a', 'path']
                tempObs = obsDir.split(os.sep)
                # We must match the dates on the files.
                if date==tempObs[-3]:
                    # We remove the observation id from the long path we formerly generated. Eg, /this/is/a/path becomes ('/this/is/a','path')
...
def telSort(telDirList, obsDirList):
    ...
    """ matches science images with the telluric images that are closest in time
		...
		Parameters:
			telDirList: list of paths to telluric images. ['path/obj/date/grat/Tellurics/obsid/']
			obsDirList: list of paths to science images. ['path/obj/date/grat/obsid/']

		Returns: Void.

		Modifications to directory structure:
    ...
    for date in dateList: # Date: YYYYMMDD
        tellist = []
        for telDir in telDirList:  # telDir: path/obj/date/grat/Tellurics/obsid
            if date in telDir:
                timeList=[]
                os.chdir(telDir)
        ...
        for a in range(len(tellist)):
            templist=[]
			# Change to a Tellurics directory
            os.chdir(tellist[a][0])
			# Open the header of the first image in the Tellurics directory (tellist[a][1][0]) is just a .fits filename.
            telheader = pyfits.open(tellist[a][1][0])
			# Grab the last three characters of the observation id
            telobs = 'obs'+ telheader[0].header['OBSID'][-3:].replace('-','')
			# Call timeCalc with the first .fits file in Telluric directory.
			# start: float. UT time of day in seconds.
			start=timeCalc(tellist[a][1][0])
			# Call timeCalc with the last .fits file in Telluric directory
			# stop: float. UT time of day in seconds.
		...
		# Iterate and change into each science directory
        for obsDir in obsDirList: # Iterate through path/obj/date/grat/obsid/ directories
            os.chdir(obsDir)
            if date in obsDir:
